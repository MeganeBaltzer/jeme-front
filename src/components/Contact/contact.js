...
const Contact = () => {
  ...
  const [isValidateContactForm, setisValidateContactForm] = useState(false);
  const form = useRef();
  ...
  const handleSubmit = (evt) => {
    ...
    if (evt.currentTarget.checkValidity() === false) {
      ...
      emailjs.send('service_dhwqdlo', 'template_3ostubr', cleanedFormData, '6GXpG7XEFNMiLD824').then(
        (result) => {
          ...
          dispatch(setisValidateContactForm(false));
          form.current.reset();
        },
        (error) => {
    ...
    dispatch(setisValidateContactForm(true));
  };

  return (
    <section className="d-flex justify-content-center" style={{ marginTop: '14em' }}>
      <div
        className=" my-5 bg-waterGreen text-white rounded p-3 p-sm-5"
        style={{ minWidth: '50%' }}
      >
        ...
        >
          ...
          <Button
            className="mt-3 mx-2 text-white"
            type="reset"
